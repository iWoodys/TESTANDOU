...
async def add_load(self, interaction: discord.Interaction,
    ...
    # Verificamos si el usuario es premium antes de permitir agregar más de 5 loadouts
    docs = list(ref.stream())  # Obtenemos todos los loadouts actuales del servidor
    if not is_premium(user_id) and len(docs) >= 3:
        await interaction.response.send_message(
            "❌ Alcanzaste el límite de 3 loadouts. Hazte premium para guardar más.",
            ephemeral=True
        )
        return

    # Guardar el nuevo loadout
